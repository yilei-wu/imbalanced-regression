...
from dfr import generate_gaussian_vectors, dfr_simple
...
class SamplingMultiTaskTrainer():
    ...
    def train(self, tasks, validation_interval, train_params, optimizer_params, resume=False, args=None):
        ...
        # dfr related
        points = generate_gaussian_vectors(1000, 128).to(self._cuda_device)
        label_range = np.linspace(0, 5, 50)
        ...
        while not should_stop:
            ...
            for batch in itertools.islice(tr_generator, 1):
                ...
                # dfr loss
                if args.dfr:
                    # print(output_dict['labels'].shape)
                    # print(output_dict['labels'])
                    # print(output_dict['embs'].shape)
                    loss_reg, loss_con, loss_uni, loss_smo = dfr_simple(output_dict['embs'], output_dict['labels'], points, label_range, self._model.FDS.running_mean_last_epoch, temperature=args.temp)
                    loss += args.w1 * loss_con + args.w2 * loss_uni + args.w3 * loss_smo
