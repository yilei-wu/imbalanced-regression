...
import random
import os
from oe import ordinal_entropy, BalancedMSELoss
...
def train(args):
    ...
    if args.dataset == "airfoil":
        ...
        train_dataset, test_dataset = get_bostondataset(5)
    else:
        raise ValueError("Invalid dataset name!")

    class_weight = torch.tensor(train_dataset.calculate_class_weights()).to(args.device)
    ...
    for epoch in range(args.epochs):
        ...
        for i, (x, y) in enumerate(train_loader):
            ...
            if args.oe:
                loss_oe = ordinal_entropy(z, y.unsqueeze(1).float())
                loss += 0.001 * loss_oe

            if args.balancedMSE:
                loss = BalancedMSELoss(class_weight)(y_hat.squeeze().float(), y.float())
        ...
        if epoch % args.print_freq == 0:
            ...
            # np.save(args.path_to_save_figures + f"saved_features_{epoch}.npy", saved_features[epoch].detach().cpu().numpy())
            # np.save(args.path_to_save_figures + f"save_labels_{epoch}.npy", save_labels[epoch].detach().cpu().numpy())
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lr_seq2seq", type=float, default=1e-5, help="Learning rate for seq2seq")
    ...
    # ordinal entropy
    parser.add_argument("--oe", action="store_true", default=False, help="Whether to run OE")

    # balanced MSE
    parser.add_argument("--balancedMSE", action="store_true", default=False, help="Whether to run balanced MSE")

    # hyper-parameter for our method
    parser.add_argument("--dfr", action="store_true", default=False, help="Whether to run DFR")
    parser.add_argument("--momentum", type=float, default=0.99, help="momentum for surrogate")
    parser.add_argument("--temperature", type=float, default=2.0, help="temperature for surrogate")
    parser.add_argument("--loas_w1", type=float, default=1.0, help="weight for contrastive loss")
    parser.add_argument("--loas_w2", type=float, default=0.4, help="weight for uniformity loss")
