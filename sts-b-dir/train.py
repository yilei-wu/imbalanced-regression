...
def main(arguments):
    ...
    parser.add_argument('--store_root', help='store root path', type=str, default='/root/checkpoint')
    ...
    parser.add_argument('--word_embs_file', help='file containing word embs', type=str, default='/root/imbalanced-regression/sts-b-dir/glove/glove.840B.300d.txt')
    ...
    parser.add_argument('--start_smooth', type=int, default=1000000, help='which epoch to start using FDS to smooth features')
    ...
    # dfr
    parser.add_argument('--dfr', action='store_true', default=False, help='whether to use DFR')
    parser.add_argument('--w1', type=float, default=0.0001, help='weight of the first term in DFR')
    parser.add_argument('--w2', type=float, default=0.01, help='weight of the second term in DFR')
    parser.add_argument('--w3', type=float, default=0.0001, help='weight of the third term in DFR')
    parser.add_argument('--temp', type=float, default=0.1, help='temperature for supervised contrastive loss')
    parser.add_argument('--projector_dim', type=int, default=128, help='dimension of the projector')

    args = parser.parse_args(arguments)

    now_time = str(datetime.datetime.now())
    now_time = '-'.join(now_time.split(' '))
    args.store_root = args.store_root

    os.makedirs(args.store_root, exist_ok=True)

    if args.dfr:
        args.store_name += f'_dfr_{args.w1}_{args.w2}_{args.w3}_{args.temp}'
    ...
    if tasks and not args.evaluate:
        ...
        trainer.train(tasks, args.val_interval, to_train, opt_params, args.resume, args)
    else:
